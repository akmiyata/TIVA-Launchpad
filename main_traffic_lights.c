...
/* Additional address pointers*/
#define PB6                 (*((volatile uint32_t *)0x40005100)) /* Base address = 0x40005000 (port B) + 0x0100 */
#define PB7                 (*((volatile uint32_t *)0x40005200)) /* Base address = 0x40005000 (port B) + 0x0200 */
#define PE2	                (*((volatile uint32_t *)0x40024010)) /* Base address = 0x40024000 (port E) + 0x0010 */
#define SYSCTL_RCC2_R       (*((volatile uint32_t *)0x400FE070)) /* PLL Initialization */
#define SYSCTL_RCC_R        (*((volatile uint32_t *)0x400FE060)) /* PLL Initialization */
#define SYSCTL_RIS_R        (*((volatile uint32_t *)0x400FE050)) /* PLL Initialization- PLLRIS */	
#define NVIC_ST_CTRL_R      (*((volatile uint32_t *)0xE000E010)) /* SYSTICK TIMER */
#define NVIC_ST_RELOAD_R    (*((volatile uint32_t *)0xE000E014)) /* SYSTICK TIMER */
#define NVIC_ST_CURRENT_R   (*((volatile uint32_t *)0xE000E018)) /* SYSTICK TIMER */
...
int main(void){ volatile uint32_t delay;
  ...
  SYSCTL_RCGC2_R |= 0x00000002; /* Enable clock for Port B */
	...
	while(1){
		/* There are three states: 1. Car senses at west light, 2. Car sensed at south light, 3. Crosswalk request, and 4. No input (no cars sensed, no crosswalk request). */
		if(PB7 == 1) {
		...
		else if(PE2 == 1){/* Crosswalk button pressed, both sets of lights need to transition to Red*/
